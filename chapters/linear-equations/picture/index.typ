...
The column picture is important in some other ways, too. We have the equation $vb(A x) = vb(b)$ to solve. If $A$ remain the same, will there be a solution for every $b$? For example, we have a three dimensional space, but the three column vectors only form a plane. Since $b$ is a linear combination of the column vectors of $A$, if $b$ is not on the plane, it's impossible to find a solution.