...
The matrix $vb(A)$ maps column vectors to column vectors, and as such, can be considered as a unary operator. As a matter of fact, the transpose is _governed_ by the identity that $ iprod( vb(A x), vb(y)) = iprod(vb(x), vb(A^top y)), $ <eq:transpose-identity-for-matrix>
