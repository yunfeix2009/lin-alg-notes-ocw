...
#lbl(<sec:fft>, heading(level: 3)[Fast Fourier Transform])
#lbl(<def:fourier>, definition[
...
When multiplying by $vb(A)$, the identity matrix part requires no real compute but the diagonal matrix part contributes to the "fix" cost, which takes $n$ steps. This process may be repeated on $vb(F)_n$. Let the complexity of $vb(F)_n$ be $C(n)$, we have $ C(n) & = 2(C(n/2)) + n/2 \
     & = 2(2 (C(n/4) + n/4)) + n/2 \
     & = 4 C(n/4) + 2 n/2. $ As this process continues, $C(1) = O(1)$, and the coefficient of $n/2$ is $log_2 n$. Thus,  $ C(n) approx 1/2 n log_2 n. $

The above method is known as _Cooley–Tukey recursion_. In the case that the dimension of the vector is not powers of $2$, this method may be generalized to treat other factorizations. In the case of the dimension being a large prime, there are still $ cases(
  "Rader's FFT (for prime lengths)",
