...
Similar to sets, vector is an ordered collection, often of real numbers, that could represent many things, including force, velocity, position, and any kind of data that describes the same object. 

The two fundamental operations of vectors, like many other algebras, are addition and (scalar) multiplication. 

When two vectors are of the same dimension, or the number of elements they hold, their sum is just the vector with each element the sum of the two vectors' corresponding elements. Additionally, when the addition defined for the elements is commutative, then vector addition is commutative. 
