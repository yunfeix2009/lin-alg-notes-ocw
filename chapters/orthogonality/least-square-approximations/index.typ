...
Let the error to be $norm(vb(b - p))$, where $vb(b)$ being the observational results and $vb(p)$ being the solvable vector, ie. in $C(vb(A))$. In the last section, it is proven that such $vb(p)$ is the projection of $vb(b)$ onto $C(vb(A))$. So, to minimize the error term that sums the squares of each individual data points' differences, the "best solution," $hat(vb(x))$ satisfies #lbl(<eqn:lsa>, $ vb(A)^top vb(A) hat(vb(x)) = vb(A)^top vb(b), $) assuming that $vb(A)^top vb(A)$, which is not only square and symmetric, but is also invertible.
...
#solution[
  ...
  Thus, $ e_Sigma & = e_1 + e_2 + e_3 \
          & = (C + D - 1)^2 + (C + 2D-2)^2 + (C + 3D - 2)^2. $
  Taking partial derivatives wrt $C$ and $D$, $ pdv(e_Sigma, C) & = 2(C + D - 1) + 2(C + 2D - 2) + 2(C + 3D-2) \
                  & = 6C + 12D - 10 = 0 \
                  & ==> 3C + 6D = 5; $
  $
    pdv(e_Sigma, D) & = 2(C + D -1 ) + 2 dot 2(C + 2D - 2) + 2 dot 3(C + 3D - 2) \
                    & = 12 C + 28 D - 22 = 0 \
                    & ==> 6C + 14D = 11.
  $
