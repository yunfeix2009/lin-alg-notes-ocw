...
As we have found above, $ vb(P) = vb(a a)^top/(vb(a)^top vb(a)). $ <eq:projectionVector>
...
Solving the matrices equation, $ hat(vb(x)) = (vb(A)^top vb(A))^(-1) vb(A)^top vb(b). $ So, the desired $ vb(p) $ is$ vb(A) (vb(A)^top vb(A))^(-1) vb(A)^top vb(b), $ and the projection matrix is $ vb(A) (vb(A)^top vb(A))^(-1) vb(A)^top. $<eq:projectionMatrix>
